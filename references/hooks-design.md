...
- **wiki-review.yml** — weekly cron, headless `claude -p "/alpha-wiki:review"` → publishes issue.
- **wiki-rollup.yml** — monthly cron, headless `claude -p "/alpha-wiki:rollup month"` → commits rollup.
